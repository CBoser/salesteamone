...
class InteractiveMenu:
    ...
    def display_main_menu(self):
        ...
        print()
        print("12. Translate Richmond Code")
        print("13. Bulk Import from CSV")
        print()
    ...
    def add_material(self):
        ...
        # Get plan code
        plan_code = input("Plan code (e.g., 1670, G603): ").strip()
        if plan_code.lower() == 'q':
            return
        
        # Offer to translate from Richmond code
        translate = input("Translate from Richmond code? [y/N]: ").strip().lower()
        
        if translate == 'y':
            richmond_pack = input("Richmond pack ID (e.g., |10.82): ").strip()
            elevation_str = input("Elevation string (e.g., 'B, C, D' or blank): ").strip()
            item_type = input("Item type (Framing/Siding): ").strip()
            
            try:
                unified_code = self.builder.translate_richmond_code(
                    plan_code, richmond_pack, elevation_str, item_type
                )
    ...
    def translate_code_interactive(self):
        """Interactive Richmond code translation"""
        self.print_header("TRANSLATE RICHMOND CODE")
        
        print("Enter Richmond code details:\n")
        
        plan_code = input("Plan code (e.g., 1670): ").strip()
        richmond_pack = input("Richmond pack ID (e.g., |10.82): ").strip()
        elevation_str = input("Elevation (e.g., 'B, C, D' or blank): ").strip()
        item_type = input("Item type (Framing/Siding): ").strip()
        ...
        try:
            ...
            print("="*80)
            print(f"Richmond Code: {richmond_pack} + {elevation_str}")
